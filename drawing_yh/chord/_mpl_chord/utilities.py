...
def compute_positions(mat, deg, in_deg, out_deg, start_at, is_sparse, kwargs,
    ...
    for i in range(num_nodes):
        ...
        for j in mat_ids[i]:
            ...
            # PATCHED: intra-sender gap — chord sub-region 两侧缩 intra_gap/2,相邻 chord 之间留 gap
            intra_gap = kwargs.get("intra_gap", 0.5)   # 度数,默认 0.5 度
            ig = intra_gap / 2
            if distsort and directed and i == j:
                pos[(i, j)] = (z0 + ig, z0 + z[j] - ig, startj + ig, startj + zj[j] - ig)
            else:
                pos[(i, j)] = (z0 + ig, z0 + z[j] - ig, startji + ig, startji + zj[jids[stop]] - ig)
