...
def chord_diagram(mat, names=None, order=None, width=0.1, pad=2., gap=0.03,
    ...
    **kwargs : keyword arguments
    ...
    # PATCHED: allow gradient + directed simultaneously
    ...
    for i in range(num_nodes):
        ...
        for j in targets:
            # PATCHED: cend = lighten(sender_color) 而不是 receiver node color
            # 让每条 chord 都从 sender_color 渐变到 lighten(sender_color),
            # 同色 chord 也能看出方向
            ...
            if mat[i, j] > 0 or (not directed and mat[j, i] > 0):
                ...
                if directed and i != j:
                    ...
                    stripe_r_outer = stripe_r_inner + stripe_thickness
                    sa, ea = (start2, end2) if start2 < end2 else (end2, start2)
                    _angs = _np.linspace(sa, ea, 50) * _np.pi / 180.
                    _outer = [(stripe_r_outer * _np.cos(_a), stripe_r_outer * _np.sin(_a)) for _a in _angs]
                    _inner = [(stripe_r_inner * _np.cos(_a), stripe_r_inner * _np.sin(_a)) for _a in _angs[::-1]]
                    _v = _outer + _inner + [_outer[0]]
                    _c = [_PA.MOVETO] + [_PA.LINETO] * (len(_v) - 2) + [_PA.CLOSEPOLY]
                    ax.add_patch(_PP(_PA(_v, _c), facecolor=sender_clr, edgecolor='none', alpha=1.0, zorder=10))
...
def chord_arc(start1, end1, start2, end2, radius=1.0, gap=0.03, pad=2,
    ...
    if directed:
        ...
        tip = 0.5*(start2 + end2)
        asize = max(gap, 0.02) * 3   # PATCHED: ×3 适中钝度
