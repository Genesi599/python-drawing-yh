...
# 读取CSV文件
df = pd.read_excel('1126-1.xlsx', index_col=0)
...
# # 提取所有大写字母
# uppercase_letters = [char for char in classname if char.isupper()]
# # 创建列表3：筛选列表2中以列表1中的大写字母开头的元素
# selected_column_names = [item for item in column_names if item[0] in uppercase_letters]

selected_column_names = ['FPKM_I98-M3-1', 'FPKM_I98-M3-2', 'FPKM_I98-M3-3', 'FPKM_NC-1', 'FPKM_NC-2', 'FPKM_NC-3']
...
cluster.ax_col_dendrogram.set_position([left, bottom + height + tree_gap*figsize_w/figsize_h, weight, tree_size*figsize_w/figsize_h])  # 顶部树状图位置
...
cluster.ax_heatmap.set_xticklabels(
    ...
    rotation=90, fontsize=15, ha='right', va='top', fontweight='bold'
)
...
# 绘制颜色条和直方图（叠加效果）
position1 = [0.05, 0.9, 0.1, 0.05*figsize_w/figsize_h]  #(left, bottom, width, height)
# 添加颜色条轴
...
cbar.set_label('Column Z-Score', fontsize=16)
# 调整刻度文字大小
cbar.ax.set_xticks(np.arange(-3, 4, 3))  # 定义刻度位置
cbar.ax.tick_params(labelsize=16)  # 设置刻度字体大小为12（你可以根据需要调整数值）
...
hist_ax.set_ylabel('Count', fontsize=16)  # 将纵轴从密度改为计数
...
height1 = 0.05
height2 = 0.05
ax.add_patch(Rectangle((xposition, height+1), width1, height1, color='black', transform=ax.transData, clip_on=False))
ax.add_patch(Rectangle((xposition+weight/2, height+1), width1, height2, color='black', transform=ax.transData, clip_on=False))

# Add text on the left of each color bar
ax.text(xposition+width1/2, height+2, 'NC', va='top', ha='center', fontsize=fontsize1, transform=ax.transData, rotation=0, fontweight='bold')
ax.text(xposition+weight/2+width1/2, height+2, '2U-miR-98-5p', va='top', ha='center', fontsize=fontsize1, transform=ax.transData, rotation=0, fontweight='bold')


plt.savefig('heatmap_with_colorbar_cluster.png', format='png', bbox_inches='tight', dpi=600)
