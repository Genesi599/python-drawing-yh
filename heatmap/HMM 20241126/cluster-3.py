...
# 读取CSV文件
df = pd.read_excel('1127.xlsx', index_col=0)
...
for i in linkage_methods:
    ...
    for a in range(0, note_number):
        print(a)
        ax.add_patch(Rectangle((xposition + a/note_number*weight, height + 1), width1, height1, color='black', transform=ax.transData, clip_on=False))

        # Add text on the left of each color bar
        ax.text(xposition + a/note_number*weight + width1/2, height + 2, namelist[a], va='top', ha='center', fontsize=fontsize1, transform=ax.transData,
