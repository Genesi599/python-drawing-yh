...
namelist = ['Young', 'Late Middle']

df = pd.read_excel('20250101.xlsx', index_col=0, sheet_name='M VS Y')
...
for i in linkage_methods:
    ...
    cluster.ax_heatmap.set_xticklabels(
        ...
        rotation=90, fontsize=20, ha='center', va='top', fontweight='bold'
    )
    ...
    # 绘制颜色条和直方图（叠加效果）
    position1 = [0.2, 0.92, 0.2, 0.05 * figsize_w / figsize_h]  #(left, bottom, width, height)
    # 添加颜色条轴
    ...
    cbar.ax.tick_params(labelsize=13)  # 设置刻度字体大小为12
    ...
    for a in range(0, note_number):
        ax.add_patch(Rectangle((xposition + a / note_number * weight, height + 0.8), width1, height1, color='black',
                               transform=ax.transData, clip_on=False))

        # Add text on the left of each color bar
        ax.text(xposition + a / note_number * weight + width1 / 2, height + 1.6, namelist[a], va='top', ha='center',
