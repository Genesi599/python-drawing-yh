...
enrich_path = base_path / "data/Pattern_Analysis/enrich_results"
...
# COMP_ID -> KEGG_ID  （用于富集）
feature_meta = pd.read_csv(base_path / "data/feature_meta.csv")
comp2kegg = feature_meta.set_index('COMP_ID')['KEGG_ID'].dropna().to_dict()
comp2hmdb = feature_meta.set_index('COMP_ID')['HMDB_ID'].dropna().to_dict()


@retry(stop=stop_after_attempt(3), wait=wait_fixed(2))
def _enrich_genes(comp_ids):
    kegg_ids = [comp2kegg[c] for c in comp_ids if c in comp2kegg]
    hmdb_ids = [comp2hmdb[c] for c in comp_ids if c in comp2hmdb]

    if len(kegg_ids) + len(hmdb_ids) < 2:
        return None

    kegg_file = Path(r"D:\Projects\ref\enrichment_dataset\KEGG\hsa_pathways_with_compounds.txt")
    smp_dir = r"D:\Projects\ref\enrichment_dataset\SMPDB\smpdb_metabolites.csv"  # 改这里
...
def enrich_and_save(expr_df, df_pat, patterns, out_dir=enrich_path):
    ...
    for i, pat in enumerate(patterns, 1):
        ...
        all_ids = expr_df.loc[genes_pat].index.tolist()  # 获取所有COMP_ID
        enrich_res = _enrich_genes(all_ids)
        gene_names = [protein_to_gene.get(g, g) for g in all_ids]  # 仅图注用
        print(f"  ├─ 转换后基因名: {gene_names}")

        print(f"  ├─ 调用Enrichr API...")
        try:
            if enrich_res is None:               # 这里已经拿到结果了
                print(f"  ├─ ⚠ API返回None")
                enrich_dict[pat] = []
                continue

            df_enrich = enrich_res.res2d  # 改为 res2d
            if df_enrich.empty:
                print(f"  ├─ ⚠ 返回空结果")
                enrich_dict[pat] = []
                continue

            df_enrich = df_enrich.sort_values('P-value').head(20)
...
kegg2name = feature_meta.set_index('KEGG_ID')['COMPOUND_Name'].to_dict()
...
def draw_heatmap(expr_df, df_pat, patterns, condition_map, age_series, enrich_dict):
    ...
    for info in reversed(pattern_info):
        ...
        if not enrich_list:
            ...
            for _, row in df_enrich.iterrows():
                ...
                # 把 KEGG ID 转成化合物名
                kegg_list = row['Genes'].split(';')[:5]
                name_list = [kegg2name.get(k, k) for k in kegg_list]
                gene_txt = ', '.join(name_list)
    ...
    out_file = OUT_DIR / "protein_patterns_heatmap_2.png"
...
def load_or_enrich(expr_df, df_pat, patterns, out_dir=enrich_path):
    # 检查是否有完整的富集结果
