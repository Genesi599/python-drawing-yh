...
classname_list = ['QuHua', 'MianYi', 'WBC', 'YanZheng']
...
for classname in classname_list:
    ...
    for i in linkage_methods:
        ...
        cluster.ax_col_dendrogram.set_position(
            [left, bottom + height + tree_gap * figsize_w / figsize_h, weight,
             tree_size * figsize_w / figsize_h])  # 顶部树状图位置
        ...
        for a in range(0, note_number):
            print(a)
            ax.add_patch(Rectangle((xposition + a / note_number * weight, height + 1), width1, height1, color='black',
                                   transform=ax.transData, clip_on=False))

            # Add text on the left of each color bar
            ax.text(xposition + a / note_number * weight + width1 / 2, height + 2, namelist[a], va='top', ha='center',
                    fontsize=fontsize1, transform=ax.transData,
                    rotation=0, fontweight='bold')

        plt.savefig(f'fig/{classname}-{i}.png', format='png', bbox_inches='tight', dpi=600)
