...
# 获取生成的对象并更新其子图位置
cluster.ax_heatmap.set_position([0.2, 0.2, 0.6, 0.6])  # [左, 下, 宽度, 高度]
cluster.ax_row_dendrogram.set_position([0.09, 0.2, 0.1, 0.6])  # 左侧树状图位置（手动增加左侧距离）
cluster.ax_col_dendrogram.set_position([0.2, 0.81, 0.6, 0.2])  # 顶部树状图位置
...
# 绘制颜色条和直方图（叠加效果）
position1 = [0.05, 0.9, 0.1, 0.05]  #(left, bottom, width, height)
# 添加颜色条轴
...
# 在热图左侧添加颜色条
xposition = weight + 1.5
width1 = 0.3
fontsize1 = 22
ax = cluster.ax_heatmap
# 在热图左侧添加颜色条
height1 = height * 8/16 - 0.5
height2 = height * 8/16 - 0.5
ax.add_patch(Rectangle((xposition, 0.25), width1, height1, color='black', transform=ax.transData, clip_on=False))
ax.add_patch(Rectangle((xposition, height*8/16 + 0.25), width1, height2, color='black', transform=ax.transData, clip_on=False))

# Add text on the left of each color bar
ax.text(xposition+0.5, 0.25+height1/2, 'Old', va='center', ha='left', fontsize=fontsize1, transform=ax.transData, rotation=0, fontweight='bold')
ax.text(xposition+0.5, height*8/16 + 0.25 + height2/2, 'Young', va='center', ha='left', fontsize=fontsize1, transform=ax.transData, rotation=0, fontweight='bold')
