...
for sheet_name in sheet_names:
    ...
    for _, row in filtered_result.iterrows():
        ...
        if row['miRNAID'] == "hsa-miR-8085":
            ...
            ax.annotate(
                ...
                xytext=(row['x']+2.5, yposition),  # Text position
                arrowprops=dict(
        ...
        if row['miRNAID'] == "hsa-miR-1285-3p":
            yposition = row['y'] + 7
            text = ax.text(
                row['x'] + 0.5,
                yposition,  # 添加y方向的偏移
            ...
            ax.annotate(
                "",  # Empty text because we're only drawing the arrow
                xy=(row['x'], row['y']),  # Point (arrow tip)
                xytext=(row['x'] + 0.5, yposition),  # Text position
                arrowprops=dict(
                    arrowstyle="->",  # Arrow type
                    color=arrow_color,  # Arrow color
                    lw=arrow_width  # Line width for the arrow
        ...
        if row['miRNAID'] == "hsa-miR-98-5p":
            ...
            ax.annotate(
                "",  # Empty text because we're only drawing the arrow
                xy=(row['x'], row['y']),  # Point (arrow tip)
                xytext=(row['x'] , yposition),  # Text position
                arrowprops=dict(
                    arrowstyle="->",  # Arrow type
                    color=arrow_color,  # Arrow color
                    lw=arrow_width  # Line width for the arrow
    ...
    plt.rcParams['figure.subplot.left'] = 0.1  # 左侧空白
    plt.rcParams['figure.subplot.right'] = 0.9  # 右侧空白
    plt.rcParams['figure.subplot.bottom'] = 0.15  # 底部空白
