...
xlabel_name = r'log$_\mathbf{2}$(o⁸G IP) in young'
...
# 分组为up, normal, down
result['group-edge'] = 'LightSteelBlue'
result['group-face'] = 'white'
...
ax.scatter(result['x'], result['y'], s=25, marker='o',
    ...
    for a in specific_names:
        if a in result['miRNAID'].tolist():
            if result.iloc[i]['miRNAID'] == a:
                if a == 'hsa-miR-500b-3p':
                    text = ax.text(
                        result.iloc[i]['x'] - 3.3,
                        result.iloc[i]['y'] + 0.05,
